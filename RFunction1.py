...
def optimise_phi(plane, event_number, thresholding=True, no_std=0.2, no_sectors=20, centre=0):
    AoI_x, AoI_z, AoI_adc, AoI_truevtxz, AoI_truevtxx = AoI(plane, event_number, thresholding, no_std)
    no_points = len(AoI_x)
    score_arr = np.zeros(no_points)
    for i in tqdm(range(no_points)):
        centre_co = (AoI_z[i], AoI_x[i])
        score_arr[i] = phi_hist(plane, event_number, True, 0.2,False, centre_co)
    
    ind = np.argmax(score_arr)
    return AoI_z[ind], AoI_x[ind]
